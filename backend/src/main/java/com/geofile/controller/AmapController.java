...
public class AmapController {
     ...
     * 获取周边地铁站点
     *
     * 接口：GET /api/amap/search-subway?location=116.4074,39.9042&radius=1000
     * 参数：location（经纬度）、radius（半径）
     * 返回：地铁站列表
     */
    @GetMapping("/search-subway")
    @Operation(summary = "搜索地铁站点", description = "搜索周边的地铁站点")
    public Result<Map<String, Object>> searchSubway(
            @Parameter(description = "经纬度", example = "116.4074,39.9042") @RequestParam String location,
            @Parameter(description = "搜索半径", example = "1000") @RequestParam(required = false) Integer radius) {

        try {
            log.info("搜索地铁站点: location={}, radius={}", location, radius);

            AmapService.POIResult[] stations = amapService.searchSubwayStations(amapKey, location, radius);

            Map<String, Object> result = new HashMap<>();
            result.put("count", stations != null ? stations.length : 0);
            result.put("stations", stations);

            log.info("找到 {} 个地铁站点", stations != null ? stations.length : 0);
            return Result.success(result);

        } catch (Exception e) {
            log.error("搜索地铁站点失败", e);
            return Result.error("搜索地铁站点失败: " + e.getMessage());
        }
    }
     ...
     * 获取周边医院
     *
     * 接口：GET /api/amap/search-hospital?location=116.4074,39.9042&radius=1000
     * 参数：location（经纬度）、radius（半径）
     * 返回：医院列表
     */
    @GetMapping("/search-hospital")
    @Operation(summary = "搜索医院", description = "搜索周边的医院")
    public Result<Map<String, Object>> searchHospital(
            @Parameter(description = "经纬度", example = "116.4074,39.9042") @RequestParam String location,
            @Parameter(description = "搜索半径", example = "1000") @RequestParam(required = false) Integer radius) {

        try {
            log.info("搜索医院: location={}, radius={}", location, radius);

            AmapService.POIResult[] hospitals = amapService.searchHospitals(amapKey, location, radius);

            Map<String, Object> result = new HashMap<>();
            result.put("count", hospitals != null ? hospitals.length : 0);
            result.put("hospitals", hospitals);

            log.info("找到 {} 个医院", hospitals != null ? hospitals.length : 0);
            return Result.success(result);
     ...
     * 获取周边餐厅
     *
     * 接口：GET /api/amap/search-restaurant?location=116.4074,39.9042&radius=1000
     * 参数：location（经纬度）、radius（半径）
     * 返回：餐厅列表
     */
    @GetMapping("/search-restaurant")
    @Operation(summary = "搜索餐厅", description = "搜索周边的餐厅")
    public Result<Map<String, Object>> searchRestaurant(
            @Parameter(description = "经纬度", example = "116.4074,39.9042") @RequestParam String location,
            @Parameter(description = "搜索半径", example = "1000") @RequestParam(required = false) Integer radius) {

        try {
            log.info("搜索餐厅: location={}, radius={}", location, radius);

            AmapService.POIResult[] restaurants = amapService.searchRestaurants(amapKey, location, radius);

            Map<String, Object> result = new HashMap<>();
            result.put("count", restaurants != null ? restaurants.length : 0);
            result.put("restaurants", restaurants);

            log.info("找到 {} 个餐厅", restaurants != null ? restaurants.length : 0);
            return Result.success(result);
