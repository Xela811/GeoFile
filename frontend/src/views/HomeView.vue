          ...
          <div v-for="file in nearbyFiles" :key="file.id" class="file-item">
            ...
            <div class="file-actions">
              <el-button
                ...
                size="small"
                :icon="Download"
                :disabled="isBtnDisabled(file) || !!quickDownloadLockMap[String(file.id)]"
          ...
          <div v-loading="previewLoading" class="preview-body">
            ...
            <template v-else-if="isInstaller(activeFile?.fileType || '')">
              <div class="installer-info-box" style="text-align: center; padding: 40px 0">
                ...
                <el-button type="primary" size="large" :disabled="!!previewDownloadLockMap[String(selectedFile?.id)]" @click="downloadFileFromPreview">
                  立即下载
                </el-button>
              </div>
            </template>
            ...
            <el-result v-else icon="warning" title="当前格式不支持在线预览">
              <template #extra>
                <el-button type="primary" :disabled="!!previewDownloadLockMap[String(selectedFile?.id)]" @click="downloadFileFromPreview">下载原文件</el-button>
              </template>
            </el-result>
          </div>
        ...
        <div v-if="selectedFile" class="file-detail">
          ...
          <div class="file-actions-dialog">
            <el-button
              ...
              size="large"
              @click="downloadFile"
              :disabled="isFileExpiredRealtime || isMaxedOut || selectedFile?.distanceExceeded || !!detailDownloadLockMap[String(selectedFile?.id)]"
            >
              <el-icon><Download /></el-icon>
              {{
...
const quickDownloadLockMap = ref<Record<string, boolean>>({})   // 对应列表快捷下载按钮 (file.id -> boolean)
const previewDownloadLockMap = ref<Record<string, boolean>>({}) // 对应预览页/弹窗里的立即下载
const detailDownloadLockMap = ref<Record<string, boolean>>({})  // 对应详情页底部的那个大按钮
...
const handleQuickDownload = (file: any) => {
  ...
  if (isBtnDisabled(file) || quickDownloadLockMap.value[file.id]) {
  ...
  nextTick(() => {
    downloadFile(file.id) // 直接调用你代码中已有的 downloadFile 方法
  })
...
// 下载文件
const downloadFile = async (calledFileId?: string) => {
  ...
  const rawId = calledFileId || selectedFile.value.id
  
  // 获取正在操作的准确文件 ID（优先用传进来的，没有就用 selectedFile 的）
  const targetFileId = String(rawId)
  
  // 检查在任意一个入口中，当前文件是否正在下载锁内。如果已经在锁里了，直接拦截，杜绝连点。
  if (
    quickDownloadLockMap.value[targetFileId] || 
    previewDownloadLockMap.value[targetFileId] || 
    detailDownloadLockMap.value[targetFileId]
  ) {
    console.log('拦截到重复点击或联动锁生效中')
    ElMessage.error('点击下载过于频繁，请稍候')
    return
  }

  // 只要突破了上面的拦截，不论你是从哪个按钮点进来的，一瞬间把该文件关联的三个入口锁全部拉满！
  quickDownloadLockMap.value = { ...quickDownloadLockMap.value, [targetFileId]: true }
  previewDownloadLockMap.value = { ...previewDownloadLockMap.value, [targetFileId]: true }
  detailDownloadLockMap.value = { ...detailDownloadLockMap.value, [targetFileId]: true }

  // 统一的解封逻辑，5秒后或拦截时无脑清空这三个 Map
  const releaseCurrentLocks = () => {
    quickDownloadLockMap.value = { ...quickDownloadLockMap.value, [targetFileId]: false }
    previewDownloadLockMap.value = { ...previewDownloadLockMap.value, [targetFileId]: false }
    detailDownloadLockMap.value = { ...detailDownloadLockMap.value, [targetFileId]: false }
  }
  ...
  try {
    ...
    if (syncData.code === 200) {
      ...
      isDetailDownloading.value = false
        ...
        setTimeout(releaseCurrentLocks, 5000)
        return
      }
...
const downloadFileFromPreview = () => {
  // 从当前的活跃文件（activeFile）中提取 ID 作为锁的 Key
  const fileId = activeFile.value?.id
  if (!fileId) return

  /*if (isPreviewDownloading.value) return
  isPreviewDownloading.value = true*/
  if (previewDownloadLockMap.value[fileId]) {
    ElMessage.warning('文件正在下载中，请勿重复点击')
    return
  }
  if (activeFile.value) {
